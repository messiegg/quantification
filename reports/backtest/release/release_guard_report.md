...
- FAIL | RG-UNIVERSE-001 | universe integrity audit | actual=FAIL | evidence=scripts/audit_universe_integrity.py | 股票池低于 floor、突破硬过滤或 override 无依据时必须 FAIL。
...
- FAIL | RG-EVIDENCE-001 | observation evidence chain | actual=FAIL | evidence=scripts/build_observation_evidence_chain.py | 若 observation/release 声称可执行，必须同时存在 evidence_chain。
...
- FAIL | RG-SENS-001 | sensitivity report binds or classifies core parameters | actual=status=FAIL; non_binding=0; unknown=0 | evidence=reports/backtest/robustness/sensitivity_report.json | 参数变化未绑定或原因 UNKNOWN 不能计入鲁棒性 PASS。
- FAIL | RG-BASELINE-001 | baseline comparison report exists and is not failing | actual=FAIL | evidence=reports/backtest/controls/baseline_comparison.json | 必须区分 documented dianjinshu-like baseline、combined_v2 和模块消融。
...
- FAIL | RG-001 | report freshness check | actual=PASS=8 WARN=0 FAIL=5 | evidence=scripts/check_report_freshness.py | 刷新有旧口径或自相矛盾的报告。
- PASS | RG-002 | release sync consistency check | actual=PASS=27 WARN=0 FAIL=0 | evidence=scripts/check_release_sync_consistency.py | 发布同步报告不得含过期结论。
- PASS | RG-003 | report path sanitization check | actual=PASS=37 WARN=0 FAIL=0 | evidence=scripts/check_report_path_sanitization.py | 公开报告不得包含本机路径或 secret。
- FAIL | RG-004 | observation gate consistency check | actual=PASS=3 WARN=0 FAIL=4 | evidence=scripts/check_observation_gate_consistency.py | observation allowed/blocked 状态必须一致。
...
- PASS | OBS-CONFLICT-001 | observation report does not keep allowed and blocked current reports together | actual=allowed=True; summary_exists=False; blocked_current=True | evidence=reports/observation/2026-05-04 | freshness 允许观察时，当前主路径不得同时保留 blocked 报告。
